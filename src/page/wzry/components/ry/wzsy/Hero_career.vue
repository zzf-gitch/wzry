<template>
  <div class="zhanji">
    <div class="cardlist1">
       <div class="HeroCard cursor-pointer" v-for="(item, index) in state.data" :key="index">
      ...
      <div class="select-mask">
        ...
        <el-button class="button" type="danger" @click="delet(item, index)"
          >删除英雄</el-button
        >
      </div>
      <img
...
<script setup>
import { getHeroLOLInfo } from "../../../../../api/heromain/hero/transfer";
import {
  ref,
  defineEmits,
  defineProps,
...
const getlist = async () => {
  await getHeroLOLInfo().then((res) => {
    console.log(res.data.heroLOL);
    console.log(res.data);
    state.data = res.data.heroLOL;
  });
};
const go = (item) => {
  router.push({ name: "heroInfo", params: { list: JSON.stringify(item) } });
};
onMounted(() => {
  getlist();
});
